...
### seleniumでの入力関数を定義
def nyuryoku(driver,a_xpath,b_naiyou,t_sleeptime) :
    texts_title = driver.find_element_by_xpath(a_xpath)
    texts_title.send_keys(b_naiyou)
    time.sleep(int(t_sleeptime))

### seleniumでのセレクト関数を定義
def select(driver,a_xpath,t_sleeptime) :
    driver.find_element_by_xpath(a_xpath).select
    time.sleep(int(t_sleeptime))

### seleniumでのクリア関数を定義
def clear(driver,a_xpath,t_sleeptime) :
